...
It does **not** mean that your Serve application, including your deployments, has already started running successfully. This happens asynchronously as the Ray cluster attempts to update itself to match the settings from your config file. Check out the [inspecting serve in production guide](serve-in-production-inspecting) to learn more about how to get the current status.
...
:::{note}
When running on a remote cluster, you need to ensure that the import path is accessible. For more details, look at how to add a runtime environment in the [handling dependencies guide](serve-handling-dependencies).
:::
...
In order to inspect the status of the serve application in production, please check out the [Inspecting Serve in production guide](serve-in-production-inspecting).
